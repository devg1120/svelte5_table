...
*/
  ...
  next_insert_row = () => {
    ...
    this.span_shift("next_insert_row", row_num);
    // span restore
    this.span_restore();
  ...
  prev_insert_row = () => {
    ...
    this.span_shift("prev_insert_row", row_num);
    // span restore
    this.span_restore();
  ...
  left_insert_col = () => {
    ...
    this.span_shift("left_insert_col", col_num);
    // span restore
    this.span_restore();
  ...
  right_insert_col = () => {
    ...
    this.span_shift("right_insert_col", col_num);
    // span restore
    this.span_restore();
  ...
  span_set = () => {
    if (this.select_td[0] != null && this.select_td[1] ) {
       ...
       console.log("span_set", this.container_id, this.table_data[r1][c1].span );
       this.span_set_remove_td(r1,c1, r2, c2);
       if ( colspan > 0 && rowspan > 0 ) {
  ...
  span_shift = (direct, num) => {

    if ( json_data[0]["span"] ) {
       //json_data[0]["sync_span"] = structuredClone(json_data[0]["span"]);
       json_data[0]["sync_span"] = JSON.parse(JSON.stringify(json_data[0]["span"]));
       let arr = json_data[0]["span"];
       for (let i = 0 ; i < arr.length; i++) {
           let r1 = parseInt(arr[i].r)
           let c1 = parseInt(arr[i].c)
           let rowspan = parseInt(arr[i].rowspan)
           let colspan = parseInt(arr[i].colspan)
	   let r2 = r1 + rowspan -1;
	   let c2 = c1 + colspan -1;
          if (direct == "left_insert_col") {
              if ( num > c1 && num<=  c2) {
                   arr[i].colspan += 1;
              } else if ( num <= c1 ) {
                   arr[i].c += 1;
              }
          } else if (direct == "right_insert_col") {
              if ( num >= c1 && num <  c2) {
                   arr[i].colspan += 1;
              } else if ( num < c1 ) {
                   arr[i].c += 1;
          ...
          } else if (direct == "prev_insert_row") {
              if ( num > r1 && num <=  r2) {
                   arr[i].rowspan += 1;
              } else if ( num <= r1 ) {
                   arr[i].r += 1;
              }
          } else if (direct == "next_insert_row") {
              if ( num >= r1 && num<  r2) {
                   arr[i].rowspan += 1;
              } else if ( num < r1 ) {
                   arr[i].r += 1;
                ...
                console.log("span_shift direct unknown", direct);
          }
       }
    } 
  }
  span_clear = () => {
    if ( json_data[0]["span"] ) {
       ...
       for (let i = 0 ; i < arr.length; i++) {
           ...
           let colspan = parseInt(arr[i].colspan)
	   let td = this.table.querySelector("#TD_" + String(r) + "_" + String(c));
	   //console.log(this.container_id,rowspan, colspan, r, c, td);
          if (colspan > 1 && rowspan  > 1) {
  ...
  sync_span_clear = () => {
    if ( json_data[0]["sync_span"] ) {
       let arr = json_data[0]["sync_span"];
       for (let i = 0 ; i < arr.length; i++) {
           let r = arr[i].r
           let c = arr[i].c
           let rowspan = parseInt(arr[i].rowspan)
 ...
 if (init) {
   ...
   if (json_data[0].update_source != container_id) {
     ...
     if (update_type == "value-change") {
        ...
        //console.log(this.container_id,"span set");
        let row_num = parseInt(param[1]);
        let col_num = parseInt(param[2]);
